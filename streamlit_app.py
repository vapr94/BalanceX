...
def chooseParentheses(temperature, count_open, count_close):
    """
    Choose a parenthesis based on the given temperature and counts.

    Args:
    temperature (float): The probability value between 0.0 and 1.0.
...
if st.button('Submit'):
    ...
    if len(matched_str) > 0:
        ...
        count_open, count_close = countParentheses(matched_str, start_string)
        ...
        while len(matched_str) > 0:
            ...
            matched_str = findMatchingStrings(start_string, corpus)
            count_open, count_close = countParentheses(matched_str, start_string)
            st.markdown(f"Open Parantheses Count= **{count_open}** , Close Paranthese Count=**{count_close}**")
            str_drawn = chooseParentheses(temperature, count_open, count_close)
