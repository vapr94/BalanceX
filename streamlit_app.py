...
def count_parentheses(matched_str, start_string):
    ...
    count_open = sum(1 for curr_str in matched_str if len(curr_str) > pos and curr_str[pos] == '(')
    count_close = sum(1 for curr_str in matched_str if len(curr_str) > pos and curr_str[pos] == ')')
...
if st.button('Submit'):
    ...
    if matched_str:
        ...
        while matched_str:
            ...
            st.markdown(f"Parentheses which has been chosen = **{str_drawn[0]}**")
